...
- 비즈니스 데이터 계층에서는 Silver와 Gold를 Iceberg 테이블로 관리합니다. 운영 메트릭 저장용 Ops table도 Iceberg입니다.
- Silver, Gold, Ops Iceberg table은 `write.merge.mode`, `write.update.mode`, `write.delete.mode`를 모두 `merge-on-read`로 설정합니다.
...
- 현재 Iceberg MERGE는 Merge-on-Read 전략입니다. 즉 MERGE/UPDATE/DELETE가 영향을 받은 data file 전체를 매번 즉시 다시 쓰기보다 delete file과 신규 data file을 만들 수 있으므로, maintenance에서 `rewrite_data_files`와 `rewrite_position_delete_files`를 함께 고려해야 합니다.
...
- MOR 전환 후에는 read path가 delete file을 함께 해석하므로, position delete file이 누적되면 BI 쿼리 성능이 나빠질 수 있습니다. compaction 정책을 수정할 때 이 점을 반드시 확인합니다.
