...
- MOR health: Merge-on-Read에서 누적될 수 있는 delete file 수 확인
...
예를 들어 `Silver freshness`는 ingestion 지연을, `small file ratio`는 data file compaction 필요성을, `delete file count`는 Merge-on-Read table의 read amplification과 position delete file compaction 필요성을, `duplicate event_id`는 멱등성 설계가 깨졌는지를 보여줍니다.
