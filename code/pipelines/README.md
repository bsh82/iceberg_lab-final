...
- `maintenance.py`: Iceberg data file compaction, position delete file compaction, expire snapshots, orphan cleanup
...
- Iceberg table은 Merge-on-Read mode를 사용하므로 MERGE 시 delete file이 누적될 수 있습니다. `maintenance.py`는 `rewrite_data_files`와 `rewrite_position_delete_files`를 함께 실행해 작은 data file과 position delete file을 정리합니다.
